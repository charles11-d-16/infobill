...
// 📍 List all Laboratory patients from Admission collection
...
// 📍 View Laboratory patient details with admission info & medical records
router.get('/laboratory/view/:id', async (req, res) => {
  ...
  try {
    ...
    // Render the Laboratory detailed view with all info
    res.render('viewLaboratoryRequest', {
      patient,
      medicalRecords,
      availableDoctors,
      availableNurses,
...
// 📍 Add diagnosis entry to a medical record for a Laboratory patient
router.post('/laboratory/view/:id/diagnose', async (req, res) => {
  const { id } = req.params;
...
// 📍 Request additional services for Laboratory patient (lab tests, x-ray, etc.)
router.post('/laboratory/view/:id/request-service', async (req, res) => {
  try {
    const {
      category,
      labSubcategory,
    ...
    const admissionData = {
      admittingId: 'laboratory' + Date.now().toString(36).toUpperCase(),
      patientType,
      fullName,
