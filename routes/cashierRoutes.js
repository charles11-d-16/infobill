...
router.get('/cashier/receipt/:paymentId', async (req, res) => {
  try {
    ...
    const receipt = await Receipt.findOne({ paymentId }).populate('patientId');
    if (!receipt) {
    ...
    res.render('cashierReceipt', {
      ...
      discountTypes: receipt.discountTypes || [],
      discountRate: receipt.discountRate || 0,
      discountAmount: receipt.discountAmount || 0,
      promissoryAmount: receipt.promissoryAmount || 0,
      finalTotal: receipt.finalTotal || 0,
      age,
      billNumber: receipt.billNumber,
      orNumber: receipt.orNumber || '',
      processedBy: receipt.processedBy || 'Cashier',
      currentDate: receipt.receiptDate || new Date(),
...
router.get('/cashier/process-receipt/:paymentId', async (req, res) => {
  try {
    ...
    res.render('processReceipt', {
      ...
      promissoryAmount: payment.promissoryAmount || 0,
      finalTotal: payment.finalTotal || 0,
      age,
      billNumber: payment.billNumber,
      orNumber: payment.orNumber || '',
      processedBy: payment.processedBy || 'Cashier',
      currentDate: payment.paymentDate || new Date(),
    ...
    console.error(err);
    res.status(500).send('Error loading processed receipt');
  }
});
