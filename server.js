...
// Apply authentication middleware globally, before all routes
...
app.post('/add-service', async (req, res) => {
  let { type, descriptions, procedureAmounts, itemUsed, itemAmounts, totalAmounts } = req.body;

  // Defensive: If any required field is missing, return error
  if (!descriptions || !procedureAmounts || !itemAmounts || !totalAmounts) {
    return res.status(400).send('Missing required service fields.');
  }

  // Ensure all fields are arrays
  if (!Array.isArray(descriptions)) descriptions = [descriptions];
  if (!Array.isArray(procedureAmounts)) procedureAmounts = [procedureAmounts];
  if (!Array.isArray(itemUsed)) itemUsed = [itemUsed];
  if (!Array.isArray(itemAmounts)) itemAmounts = [itemAmounts];
  if (!Array.isArray(totalAmounts)) totalAmounts = [totalAmounts];
